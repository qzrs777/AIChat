...
从本版本开始，Mod 支持“持续通话”模式：开启后，麦克风会保持监听，检测到你说完一句话后自动识别、调用 AI、播放语音回复；AI 说完后又会自动恢复监听。

### 开启方式
1. 进入游戏，按 `F9` / `F10` 打开 Mod 界面。
2. 点击 **“☎️ 开始持续通话”** 按钮，或展开设置面板的 **“持续通话配置”** 勾选启用。
3. 直接对着麦克风说话即可。
...
| 配置项 | 说明 | 默认值 |
|--------|------|--------|
| `Enable_VoiceCall` | 是否启用持续通话 | `false` |
| `Vad_EnergyThreshold` | 语音检测能量阈值，越小越灵敏 | `0.02` |
| `Vad_MinSpeechSeconds` | 最短有效语音时长（秒），用于过滤突发噪声 | `0.4` |
| `Vad_SilenceSeconds` | 停顿多久判定为说话结束（秒） | `1.0` |
| `VoiceCall_BargeIn` | 是否允许在 AI 说话时打断 | `true` |
| `VoiceCall_ResumeDelay` | AI 说完后多久恢复监听（秒） | `0.5` |

### 使用建议
- **建议佩戴耳机**：当前实现没有回声消除（AEC），如果扬声器声音被麦克风回采，可能会触发误识别或自循环。
- **调节 VAD 阈值**：环境较吵时适当调大 `Vad_EnergyThreshold`；说话较轻或离麦克风较远时适当调小。
- **API 费用**：持续通话会每轮都调用 LLM API，长时间使用请注意 token 消耗。
